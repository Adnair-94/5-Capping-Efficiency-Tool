# RNA Forge 5' Capping Efficiency Tool — desktop version v11
...
- Optional automatic exclusion of not-found species from the efficiency calculation.
- Optional bounded calls: `>99%` when capped species are detected but no included uncapped species are found, and `<1%` when uncapped species are detected but no included capped species are found.


## v11 change

- Added **Combine capped z=1/z=2 in composite plot** option.
- When enabled, capped species that resolve to the same neutral mass are summed into one capped EIC trace for the final composite Plotly plot and composite TIFF export.
- This is a visualization/export feature only. The capping-efficiency calculation remains based on the AUC table: summed capped AUC divided by summed capped plus uncapped AUC.
- Charge-state matching uses explicit `charge` and `neutral_mass` columns when present. If they are missing, the app attempts z=1/z=2 neutral-mass matching from m/z values.
