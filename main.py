...
Version 9 changes:
- Adds minimum contiguous EIC-point requirement for RT-window AUC integration.
- Adds Plotly x-axis range slider for EIC inspection.
- Adds co-eluted labile phosphate-loss screening to ignore lower-phosphate uncapped species.
- Adds final composite EIC plot/export for included capped and uncapped species.
"""
...
class CappingEfficiencyWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        input_layout.addWidget(QLabel("Species mode"), 10, 0)
        input_layout.addWidget(self.mode_combo, 10, 1, 1, 2)
        input_layout.addWidget(QLabel("RT display unit"), 11, 0)
        input_layout.addWidget(self.rt_unit_combo, 11, 1, 1, 2)
...
"""
    ...
    def _save_selected_window_from_js(self, result):
        try:
            ...
            self._set_status(
                ...
                f"AUC = {auc:.4g}; included points Intensity > {min_i:.4g}; min contiguous points = {min_pts}"
            )
            self.refresh_tables()
            self.plot_selected_species()
        except Exception as exc:
    ...
    def _composite_plotly_html(self, rt_unit: str = "seconds") -> str:
        ...
        state = self.session.state(self.requested_mode())
        auc_tbl = auc_table(self.session, self.requested_mode())
        if auc_tbl is None or len(auc_tbl) == 0:
            return self._blank_html("No species available for composite plot")
        include_col = "use_for_efficiency_effective" if "use_for_efficiency_effective" in auc_tbl.columns else "use_for_efficiency"
        include = auc_tbl[include_col].astype(str).str.lower().isin(["true", "1", "yes"])
        plot_tbl = auc_tbl.loc[include & auc_tbl["type"].astype(str).isin(["Capped", "Uncapped"])].copy()
        fig = go.Figure()
...
"""
    ...
    def show_composite_plot(self):
        ...
        if WEBENGINE_AVAILABLE:
            try:
                ...
                self._set_status("Composite EIC plot shown. It includes effective included capped and uncapped species; labile phosphate-loss species flagged for exclusion are omitted.")
            except Exception as exc:
    ...
    def export_composite_tiff(self):
        ...
        if path:
            export_composite_eic_tiff(self.session, self.requested_mode(), path, rt_unit=self.rt_display_unit())
