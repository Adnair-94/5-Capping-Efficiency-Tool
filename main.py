...
Version 11 changes:
- Adds an option to combine capped z=1/z=2 charge-state EICs in the final composite plot.
- The summed capped trace is for visualization/export; capping efficiency remains AUC-sum based from the AUC table.
"""
...
class CappingEfficiencyWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.combine_capped_charges = QCheckBox("Combine capped z=1/z=2 in composite plot")
        self.combine_capped_charges.setChecked(False)
    ...
    def show_composite_plot(self):
        ...
        if WEBENGINE_AVAILABLE:
            try:
                ...
                if self.combine_capped_charges.isChecked():
                    self._set_status("Composite EIC plot shown. Included capped z=1/z=2 charge-state traces with matching neutral mass are summed into one capped trace. Labile phosphate-loss species flagged for exclusion are omitted.")
                else:
