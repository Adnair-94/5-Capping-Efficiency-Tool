...
Version 10 changes:
- Allows species not found beyond user-set limits to be ignored automatically.
- Reports bounded capping calls (>99% or <1%) when only one class is detected.
- Keeps all failed/not-found species visible in exported AUC tables with status columns.
"""
...
class CappingEfficiencyWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        input_layout.addWidget(QLabel("Species mode"), 12, 0)
        input_layout.addWidget(self.mode_combo, 12, 1, 1, 2)
        input_layout.addWidget(QLabel("RT display unit"), 13, 0)
        input_layout.addWidget(self.rt_unit_combo, 13, 1, 1, 2)
...
"""
    ...
    def _composite_plotly_html(self, rt_unit: str = "seconds") -> str:
        ...
        include_col = "included_for_efficiency_final" if "included_for_efficiency_final" in auc_tbl.columns else ("use_for_efficiency_effective" if "use_for_efficiency_effective" in auc_tbl.columns else "use_for_efficiency")
