...
class CappingEfficiencyWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.plot_hint = QLabel(
            ...
            "The selected x-range is used as the RT window. AUC uses only points above the AUC point minimum intensity."
        )
...
"""
    ...
    def _save_selected_window_from_js(self, result):
        try:
            ...
            self._set_status(f"Window for {species} saved: [{rt_min:.2f}, {rt_max:.2f}] seconds; AUC = {auc:.4g}; included points Intensity > {min_i:.4g}")
            self.refresh_tables()
            self.plot_selected_species()
        except Exception as exc:
