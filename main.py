...
Version 4 changes:
- Loads Plotly EIC HTML from a temporary local file instead of QWebEngine.setHtml.
  This avoids Qt/WebEngine data-URL size limits with inline plotly.js in PyInstaller builds.
- Keeps embedded Plotly box/lasso selection closer to the original Shiny/plotly app.
- Keeps scientific logic in capping_core.py.
"""
...
class CappingEfficiencyWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        input_layout.addWidget(QLabel("Species mode"), 4, 0)
        input_layout.addWidget(self.mode_combo, 4, 1, 1, 2)
        ...
        self.plot_hint = QLabel(
            ...
            "The selected x-range is used as the RT window."
        )
    ...
    def _build_exports_tab(self):
        ...
        for btn in [export_rt_btn, export_auc_btn, export_excel_btn, export_tiff_btn, export_zip_btn]:
    ...
    # ------------------------------------------------------------------
    def _plotly_html(self, eic: pd.DataFrame, species: str, rt_window=None) -> str:
        ...
        if eic is not None and len(eic) > 0:
            fig.add_trace(
                go.Scatter(
                    x=pd.to_numeric(eic["RT"], errors="coerce"),
...
"""
    ...
    def plot_selected_species(self):
        ...
        if WEBENGINE_AVAILABLE:
            try:
                self._load_html_in_plot_view(self._plotly_html(eic, species, rt_window=rt_window))
            except Exception as exc:
    ...
    def _save_selected_window_from_js(self, result):
        try:
            ...
            rt_min, rt_max = float(selected[0]), float(selected[1])
            if rt_min == rt_max:
                raise ValueError("Selection has zero width.")
            species = self.selected_species_combo.currentText()
            auc = save_rt_window(self.session, self.requested_mode(), species, rt_min, rt_max)
            self._set_status(f"Window for {species} saved: [{rt_min:.2f}, {rt_max:.2f}] seconds; AUC = {auc:.4g}")
            self.refresh_tables()
            self.plot_selected_species()
        except Exception as exc:
