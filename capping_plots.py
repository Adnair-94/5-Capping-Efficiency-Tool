...
def plot_composite_eics(ax, eic_map: dict, species_table: pd.DataFrame, rt_windows: Optional[pd.DataFrame] = None, rt_unit: str = "seconds", x_range_display=None, y_range=None) -> None:
    ...
    ax.set_xlabel(_rt_label(rt_unit))
    ax.set_ylabel("Intensity")
    if x_range_display is not None and len(x_range_display) == 2:
        try:
            ax.set_xlim(float(x_range_display[0]), float(x_range_display[1]))
        except Exception:
            pass
...
def save_composite_eic_tiff(eic_map: dict, species_table: pd.DataFrame, output_path: str | Path, rt_windows: Optional[pd.DataFrame] = None, dpi: int = 300, rt_unit: str = "seconds", x_range_display=None, y_range=None) -> None:
    fig, ax = plt.subplots(figsize=(9, 5.5))
    plot_composite_eics(ax, eic_map, species_table, rt_windows=rt_windows, rt_unit=rt_unit, x_range_display=x_range_display, y_range=y_range)
    fig.savefig(output_path, dpi=dpi, format="tiff", bbox_inches="tight")
    plt.close(fig)
