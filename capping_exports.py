...
def export_excel_workbook(
    ...
    with pd.ExcelWriter(path, engine="xlsxwriter") as writer:
        ...
        try:
            from capping_core import combine_capped_charge_state_auc_table
            calc_tbl = combine_capped_charge_state_auc_table(auc_tbl) if combine_capped_charge_states else auc_tbl
            calc_tbl.to_excel(writer, index=False, sheet_name="auc_table_for_efficiency")
        except Exception:
            pass
...
def export_composite_eic_tiff(
    ...
    save_composite_eic_tiff(
        eic_map,
        species_table,
        path,
        rt_windows=state.rt_windows,
        dpi=dpi,
        rt_unit=rt_unit,
        x_range_display=x_range_display,
        y_range=y_range,
    )
