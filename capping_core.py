...
def save_rt_window(
    ...
    if not bool(qc["AUC_peak_passed_min_points"]):
        raise ValueError(
            f"Selected RT window did not contain a peak with at least {int(min_consecutive_points)} contiguous points "
            f"above intensity {float(min_intensity):.4g}. Max contiguous points found: {int(qc['AUC_max_consecutive_points'])}."
        )
    return auc
...
def calculate_efficiency(auc_tbl: pd.DataFrame, honor_use_for_efficiency: bool = True) -> Dict[str, float | str | bool]:
    ...
    if honor_use_for_efficiency:
        if "use_for_efficiency_effective" in df.columns:
            include = as_logical_safely(df["use_for_efficiency_effective"], default=True).values
        elif "use_for_efficiency" in df.columns:
            include = as_logical_safely(df["use_for_efficiency"], default=True).values
        else:
            include = np.array([True] * len(df))
