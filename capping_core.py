...
def save_rt_window(session: AnalysisSession, requested_mode: str, species_name: str, rt_min: float, rt_max: float, min_intensity: float = 0.0) -> float:
    ...
    if "RTmin_min" not in state.rt_windows.columns:
        state.rt_windows["RTmin_min"] = np.nan
    if "RTmax_min" not in state.rt_windows.columns:
        state.rt_windows["RTmax_min"] = np.nan
